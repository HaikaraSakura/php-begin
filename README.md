...
PHPはWebサイトを簡単に作れるよう、Web関連の機能が充実しています。`$_GET`、`$_POST`の存在はそのひとつです。    
これらのスーパーグローバル変数はユーザーのリクエストに含まれるパラメータを取り出した値が配列として入っていますが、   
どこからでも書き換え可能で、テストがしづらいというデメリットがあり、現代的なフレームワークでは利用されません。

PHPはWeb向けの言語であり、HTTP関連の標準関数がたくさんありますが、
現代的なフレームワークではRequest/Responseを抽象化した機能を備えているため、PHP自体に備わっている機能を直接扱うことは稀です。  

また、特定のフレームワークへの依存を避けるための標準的な規格としてPSRというものもあります。  
...
「このファイルの中で`ResponseFactory`といえば`Laminas\Diactoros\ResponseFactory`のことだぞ」と宣言していることになります。  
クラスを使うたびに完全修飾名（名前空間を含むフルネーム）を書くのは面倒ですが、use宣言をしておけば省略できるのです。  

なお、細かい話ですが、use自体にファイルを読み込む機能があるわけではありません。  
詳しいことが知りたい場合は、名前空間とComposerのオートロードについて調べてみてください。
...
URLのクエリパラメータに`?id=1`と指定するとidが1の商品の情報が表示される、といったものです。

PHPでは`$_GET`や`$_POST`などのスーパーグローバル変数というものが存在し、`$_GET['id']`などとすると値を取得できますが、  
現代的なフレームワークにはRequest/Responseを抽象化した機能が備わっているので、これらのスーパーグローバル変数を使用することはありません。
今回は特定のフレームワークは使いませんが、PSRという標準的な規格があるので、それに準拠したライブラリを用いることとします。

Request/Responseに関してはPSR-7として定義されています。  
その実装である`laminas/laminas-diactoros`はすでにComposerでインストール済みです。  
PSR-7でRequestからURLのぱらめーたを取り出すには、以下のように記述します。  
...
`$_GET`の代わりに`$request->geQueryParams()`で`Request`からクエリパラメータを取り出しているのが分かるでしょうか？
もしURLに`name`を指定していない場合は、代わりに`こんにちは世界！`と表示されるはずです。
...
ルーティングコールバックとして、Actionクラスをインスタンス化して渡している点です。  
これでは、どのルートのコールバックが実行されたかにかかわらず、すべてのActionクラスのコンストラクタが実行されるので、  
パフォーマンス的にもよくありませんし、意図しない処理が実行されることで思わぬ副作用が生じかねません。
